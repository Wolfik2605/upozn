...
function CheckPlayerName(Name: string; PlayerList: PPlayerList)
  : boolean; overload;
var
...
begin
  ...
  while (CurrentPlayer <> nil) and not Result do
  begin
    if UpperCase(CurrentPlayer^.Info.Name) = UpperCase(Name) then
      Result := true;
    CurrentPlayer := CurrentPlayer^.Ptr;
  end;
end;

function CheckTeamName(Name: string; TeamList: PTeamList): boolean; overload;
var
  CurrentTeam: PTeamList;
begin
  Result := false;
  CurrentTeam := TeamList^.Ptr;
  while (CurrentTeam <> nil) and not Result do
  begin
    if UpperCase(CurrentTeam^.Info.Name) = UpperCase(Name) then
      Result := true;
...
function CheckCode(Code: integer; PlayerList: PPlayerList;
  CheckAsTeamCode: boolean): boolean;
overload
begin
  Result := false;
...
begin
  Writeln(Format('|%5d|%-27s|%-22s|%11d|', [TeamInfo.Code, TeamInfo.Name,
    TeamInfo.Country, TeamInfo.TableNumber]));
end;

procedure WriteInfo(PlayerInfo: TPlayerInfo); overload;
begin
  Writeln(Format('|%5d|%-25s|%6d|%-12s|%7d|%13d|', [PlayerInfo.Code,
    PlayerInfo.Name, PlayerInfo.TeamCode, PlayerInfo.Position,
    PlayerInfo.Success, PlayerInfo.PenaltyPoints]));
end;
...
begin
  ...
  begin
    Writeln('|--------------------------------------------------------------------|');
    Writeln(Format('| %-3s | %-25s | %-20s | %9s |', ['Код', 'Название',
      'Страна', 'Таблица']));
    Writeln('|-----|---------------------------|----------------------|-----------|');
    repeat
      TeamList := TeamList^.Ptr;
...
begin
  ...
  begin
    Writeln('|-------------------------------------------------------------------------|');
    Writeln(Format('|%-5s|%-25s|%-6s|%-12s|%-7s|%-13s|', ['Код', 'ФИО', 'КодК',
      'Амплуа', 'Успех', 'Штраф. очки']));
    Writeln('|-----|-------------------------|------|------------|-------|-------------|');
    repeat
      PlayerList := PlayerList^.Ptr;
...
begin
  ...
  begin
    ...
    begin
      PlayerList := PlayerList^.Ptr;
      if not(Compare(PlayerList^.Info, Key) or Compare(Key, PlayerList^.Info))
      then
      begin
        WriteInfo(PlayerList^.Info);
      end;
...
procedure SearchTeam(TeamList: PTeamList; Compare: TTeamComparator;
  Key: TTeamInfo);
begin
  if TeamList <> nil then
  begin
    Writeln('|---------------------------------------------------------------------------|');
    Writeln(Format('|%-5s|%-25s|%-6s|%-12s|%-7s|%-13s|', ['Код', 'Название',
      'Страна', 'Таблица']));
    Writeln('|-----|-------------------------|------|------------|-------|-------------|');
    while TeamList^.Ptr <> nil do
    begin
...
procedure ReadPlayer(var TeamCode: integer; var Name, Position: string;
  var Success, PenaltyPoints: integer; TeamList: PTeamList);
var
  Error: boolean;
begin
  repeat
...
procedure AddElement(PlayerList: PPlayerList; TeamList: PTeamList;
  var Max: integer);
overload
var
...
begin
  ...
  begin
    ...
    if CheckPlayerName(Name, PlayerList) then
    begin
      Writeln('Ошибка: футболист с таким ФИО уже существует.');
      Writeln;
    end
    else
    begin
...
begin
  ...
  if CheckTeamName(Name, TeamList) then
  begin
    Writeln('Ошибка: команда с таким названием уже существует.');
    Writeln;
  end
  else
  begin
...
procedure RedactElement(PlayerList: PPlayerList; Code: integer;
  TeamList: PTeamList; Field: integer);
overload
var
  TeamCode, Success, PenaltyPoints: integer;
  Name, Position: string;
...
procedure DeleteElement(TeamList: PTeamList; Code: integer;
...
begin
  ...
  begin
    ...
    if not CheckCode(Code, PlayerList, true) then
    begin
      IsFound := false;
...
procedure CalculateMax(PlayerList: PPlayerList; TeamList: PTeamList;
  out PMax, TMax: integer);
begin
  PMax := 0;
  TMax := 0;
  while PlayerList^.Ptr <> nil do
            ...
            1:
              begin
                ...
                begin
                  ...
                  case Input2 of
                    0:
                      SortTeam(TeamList, TeamCodeComparator);
                    1:
                      SortTeam(TeamList, TeamNameComparator);
                    2:
                      SortTeam(TeamList, TeamCountryComparator);
                    3:
                      SortTeam(TeamList, TeamTableNumberComparator);
                  end;
                  WriteList(TeamList);
            ...
            2:
              begin
                ...
                begin
                  ...
                  case Input2 of
                    0:
                      SortPlayer(PlayerList, PlayerCodeComparator);
                    1:
                      SortPlayer(PlayerList, PlayerNameComparator);
                    2:
                      SortPlayer(PlayerList, PlayerSuccessComparator);
                    3:
                      SortPlayer(PlayerList, PlayerPenaltyComparator);
                    4:
                      SortPlayer(PlayerList, PlayerTeamCodeComparator);
                  end;
                  WriteList(PlayerList);
                end
                else
                  ...
                  case Input2 of
                    ...
                    3:
                      begin
                        ...
                        SearchTeam(TeamList, TeamTableNumberComparator,
                          TeamKey);
                      end;
                  end;
                end
                  ...
                  case Input2 of
                    0:
                      begin
                        ...
                        SearchPlayer(PlayerList, PlayerCodeComparator,
                          PlayerKey);
                      end;
                    1:
                      begin
                        ...
                        SearchPlayer(PlayerList, PlayerNameComparator,
                          PlayerKey);
                      end;
                    2:
                      begin
                        ...
                        SearchPlayer(PlayerList, PlayerSuccessComparator,
                          PlayerKey);
                      end;
                    3:
                      begin
                        ...
                        SearchPlayer(PlayerList, PlayerPenaltyComparator,
                          PlayerKey);
                      end;
                    4:
                      begin
                        ...
                        SearchPlayer(PlayerList, PlayerTeamCodeComparator,
                          PlayerKey);
                      end;
                  end;
                end
        ...
        begin
          ...
          case Input of
              ...
              begin
                ...
                begin
                  ...
                  OutputContent := OutputContent +
                    Format('|%-5s|%-25s|%-6s|%-12s|%-7s|%-13s|',
                    ['Код', 'ФИО', 'КодК', 'Амплуа', 'Успех', 'Штраф. очки']
                    ) + #13#10;
                  OutputContent := OutputContent +
                    '|-----|-------------------------|------|------------|-------|-------------|'
                    + #13#10;
                  ...
                  Writeln('|-------------------------------------------------------------------------|');
                  Writeln(Format('|%-5s|%-25s|%-6s|%-12s|%-7s|%-13s|',
                    ['Код', 'ФИО', 'КодК', 'Амплуа', 'Успех', 'Штраф. очки']));
                  Writeln('|-----|-------------------------|------|------------|-------|-------------|');
                  Input2_local := 0;
                  CurrentPlayer := PlayerList^.Ptr;
                  ...
                  begin
                    ...
                    OutputContent := OutputContent +
                      Format('|%5d|%-25s|%6d|%-12s|%7d|%13d|' + #13#10,
                      [CurrentPlayer^.Info.Code, CurrentPlayer^.Info.Name,
                      CurrentPlayer^.Info.TeamCode,
                      CurrentPlayer^.Info.Position, CurrentPlayer^.Info.Success,
                      CurrentPlayer^.Info.PenaltyPoints]);
                    CurrentPlayer := CurrentPlayer^.Ptr;
                    Inc(Input2_local);
                  end;
                  OutputContent := OutputContent +
            ...
            2: // Топ-10 по штрафным очкам
              begin
                ...
                begin
                  ...
                  OutputContent :=
                    'Топ-10 игроков с максимальной суммой штрафных очков:' +
                    #13#10;
                  OutputContent := OutputContent +
                    Format('|%-5s|%-25s|%-6s|%-12s|%-7s|%-13s|',
                    ['Код', 'ФИО', 'КодК', 'Амплуа', 'Успех', 'Штраф. очки']
            ...
            3: // Самый успешный и по штрафным очкам для каждой команды
              begin
                OutputContent :=
                  'Самый успешный игрок и игрок с максимальной суммой штрафных очков для каждой команды:'
                  + #13#10#13#10;
                ...
                begin
                  ...
                  begin
                    ...
                    OutputContent := OutputContent + 'Команда: ' +
                      CurrentTeam^.Info.Name + #13#10;
                    ...
                    begin
                      if TempPlayerList^.Info.TeamCode = CurrentTeam^.Info.Code
                      then
                      begin
                        FoundPlayersForTeam := true;
                        if TempPlayerList^.Info.Success > BestPlayer.Success
                        then
                          BestPlayer := TempPlayerList^.Info;

                        if TempPlayerList^.Info.PenaltyPoints >
                          MostPenaltyPlayer.PenaltyPoints then
                    ...
                    begin
                      ...
                      OutputContent := OutputContent +
                        '  Самый успешный игрок:' + #13#10;
                      OutputContent := OutputContent +
                        Format('|%5d|%-25s|%6d|%-12s|%7d|%13d|' + #13#10,
                        [BestPlayer.Code, BestPlayer.Name, BestPlayer.TeamCode,
                        BestPlayer.Position, BestPlayer.Success,
                        BestPlayer.PenaltyPoints]);

                      Writeln('  Игрок с максимальными штрафными очками:');
                      WriteInfo(MostPenaltyPlayer);
                      OutputContent := OutputContent +
                        '  Игрок с максимальными штрафными очками:' + #13#10;
                      OutputContent := OutputContent +
                        Format('|%5d|%-25s|%6d|%-12s|%7d|%13d|' + #13#10,
                        [MostPenaltyPlayer.Code, MostPenaltyPlayer.Name,
                        MostPenaltyPlayer.TeamCode, MostPenaltyPlayer.Position,
                        MostPenaltyPlayer.Success,
                        MostPenaltyPlayer.PenaltyPoints]);
                    end
                    else
                    begin
                      Writeln('  В этой команде нет футболистов.');
                      OutputContent := OutputContent +
                        '  В этой команде нет футболистов.' + #13#10;
                    end;
                    Writeln;
